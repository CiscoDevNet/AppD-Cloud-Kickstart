...
data "azurerm_shared_image_version" "teastore" {
...
resource "azurerm_network_security_rule" "allow_teastore" {
  name                        = "security-rule-allow-teastore-${var.azurerm_resource_name_prefix}-${local.current_date}"
  priority                    = 103
  direction                   = "Inbound"
  access                      = "Allow"
  protocol                    = "Tcp"
...
resource "azurerm_public_ip" "teastore" {
...
resource "azurerm_network_interface" "teastore" {
  ...
  ip_configuration {
    ...
    public_ip_address_id          = azurerm_public_ip.teastore[count.index].id
  }

  tags = var.resource_tags
}

resource "azurerm_network_interface_security_group_association" "teastore" {
  count                     = var.lab_count
  network_interface_id      = azurerm_network_interface.teastore[count.index].id
  network_security_group_id = azurerm_network_security_group.nsg.id
}

resource "azurerm_linux_virtual_machine" "teastore" {
  ...
  resource_group_name   = data.azurerm_resource_group.cloud_workshop.name
  location              = data.azurerm_resource_group.cloud_workshop.location
  computer_name         = format("%s-%02d-vm", var.azurerm_teastore_vm_hostname_prefix, count.index + local.start_number)
  size                  = var.azurerm_vm_size
  admin_username        = var.azurerm_ssh_username
  network_interface_ids = [azurerm_network_interface.teastore[count.index].id]
  ...
  custom_data = base64encode(templatefile("${path.module}/templates/user-data-sh.tmpl", {
    ...
    azurerm_vm_hostname_prefix = var.azurerm_teastore_vm_hostname_prefix,
    azurerm_vm_domain          = "",
  ...
  source_image_id = data.azurerm_shared_image_version.teastore.id
  tags            = var.resource_tags
}
...
resource "null_resource" "ansible_trigger" {
  ...
  triggers = {
    compute_instance_ids = join(",", azurerm_linux_virtual_machine.teastore.*.id)
  }
...
[teastore_vm]
${join("\n", toset(azurerm_linux_virtual_machine.teastore.*.public_ip_address))}
EOF
EOD
  }
}
