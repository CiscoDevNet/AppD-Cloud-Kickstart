...
data "azurerm_image" "lpad" {
  name_regex          = "LPAD-CentOS79-Image-*"
  sort_descending     = true
  resource_group_name = "EdBarberis"
}
...
resource "azurerm_public_ip" "lpad" {
  name  = format("%s-%02d-public-ip-%s", var.azurerm_lpad_vm_hostname_prefix, count.index + local.start_number, local.current_date)
  count = var.lab_count
...
resource "azurerm_network_interface" "lpad" {
  name  = format("%s-%02d-network-interface-%s", var.azurerm_lpad_vm_hostname_prefix, count.index + local.start_number, local.current_date)
  count = var.lab_count
...
resource "azurerm_linux_virtual_machine" "lpad" {
  name  = format("%s-%02d-vm-%s", var.azurerm_lpad_vm_hostname_prefix, count.index + local.start_number, local.current_date)
  count = var.lab_count
