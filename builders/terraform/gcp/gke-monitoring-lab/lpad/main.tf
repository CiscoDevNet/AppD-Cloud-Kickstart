...
module "instance_template" {
  ...
  metadata = {
    ...
    startup-script = templatefile("${path.module}/templates/user-data-sh.tmpl", {
      ...
      gcp_lpad_vm_hostname_prefix = var.gcp_lpad_vm_hostname_prefix,
      gcp_lpad_vm_domain          = "",
...
module "lpad_vm" {
  ...
  network           = google_compute_network.vpc.name
  subnetwork        = google_compute_subnetwork.vpc-public-subnet-01.name
  hostname          = var.gcp_lpad_vm_hostname_prefix
...
resource "google_compute_firewall" "allow-lpad" {
  name    = "firewall-rule-allow-lpad-${var.gcp_resource_name_prefix}-${local.current_date}"
  network = google_compute_network.vpc.name
...
resource "null_resource" "ansible_trigger" {
  ...
  triggers = {
    gcp_instance_ids = join(",", concat(module.lpad_vm.instance_id))
  }
