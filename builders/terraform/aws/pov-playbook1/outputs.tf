...
output "aws_ec2_pov_playbook1_vm_hostname_prefix" {
  description = "AWS EC2 PoV Playbook1 VM hostname prefix."
  value       = var.aws_ec2_pov_playbook1_vm_hostname_prefix
}
...
output "public_ips" {
  ...
  value       = flatten([for vm in module.pov_playbook1_vm : vm.public_ip])
}

output "public_dns" {
  description = "List of public DNS names assigned to the instances."
  value       = flatten([for vm in module.pov_playbook1_vm : vm.public_dns])
}
...
output "resource_tags" {
  ...
  value       = flatten([for vm in module.pov_playbook1_vm : vm.tags_all])
}
