...
locals {
  ...
  security_group_name       = "${var.resource_name_prefix}-${local.unique_string}-Security-Group"
  ec2_access_role_name      = "${var.resource_name_prefix}-${local.unique_string}-EC2-Access-Role"
  ec2_access_policy_name    = "${var.resource_name_prefix}-${local.unique_string}-EC2-Access-Policy"
  ec2_instance_profile_name = "${var.resource_name_prefix}-${local.unique_string}-EC2-Instance-Profile"

  # define resource tagging here to ensure standardized naming conventions.
  # pov playbook1 tag names for aws resources.
  pov_playbook1_resource_tags = {
  ...
  # merge 'pov_playbook1_resource_tags' and 'appd_resource_tags' into one set.
  resource_tags = merge(local.pov_playbook1_resource_tags, local.appd_resource_tags)
}
...
data "aws_ami" "pov_playbook1_ami" {
  most_recent = true
  owners      = ["self"]
...
module "pov_playbook1_vm" {
  ...
  name                 = var.lab_count > 1 || var.lab_use_num_suffix ? "${var.resource_name_prefix}-${each.key}-${local.unique_string}-VM" : "${var.resource_name_prefix}-${local.unique_string}-VM"
  ami                  = data.aws_ami.pov_playbook1_ami.id
...
#   capacity_reservation_preference = "open"
  ...
  user_data_base64 = base64encode(templatefile("${path.module}/templates/user-data-sh.tmpl", {
    ...
    aws_ec2_hostname            = var.lab_count > 1 || var.lab_use_num_suffix ? "${var.aws_ec2_pov_playbook1_vm_hostname_prefix}-${each.key}-vm" : "${var.aws_ec2_pov_playbook1_vm_hostname_prefix}-vm"
...
resource "null_resource" "ansible_trigger" {
  ...
  triggers = {
    ec2_instance_ids = join(",", flatten([for vm in module.pov_playbook1_vm : vm.id]))
  }
...
[pov_playbook1_vm]
${join("\n", flatten([for vm in module.pov_playbook1_vm : vm.public_dns]))}
EOF
EOD
  }
}
