...
output "aws_ec2_lpad_vm_hostname_prefix" {
  description = "AWS EC2 LPAD VM hostname prefix."
  value       = var.aws_ec2_lpad_vm_hostname_prefix
}
...
output "public_dns" {
  ...
  value       = flatten([for vm in module.lpad_vm : vm.public_dns])
}
...
output "resource_tags" {
  ...
  value       = flatten([for vm in module.lpad_vm : vm.tags_all])
}
