...
output "aws_ec2_apm_platform_vm_hostname_prefix" {
  description = "AWS EC2 APM Platform VM hostname prefix."
  value       = var.aws_ec2_apm_platform_vm_hostname_prefix
}
...
output "public_ips" {
  ...
  value       = flatten([for vm in module.apm_platform_vm : vm.public_ip])
}

output "public_dns" {
  description = "List of public DNS names assigned to the instances."
  value       = flatten([for vm in module.apm_platform_vm : vm.public_dns])
}
...
output "resource_tags" {
  ...
  value       = flatten([for vm in module.apm_platform_vm : vm.tags_all])
}
